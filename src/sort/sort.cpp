    ...
    if (start_idx < end_idx)
    {
        ...
        while (left_ptr <= right_ptr)
        {
            ...
            while (arr[left_ptr] < pivot)
            {
                left_ptr++;
                std::unordered_map<uint16_t, std::string> color_map{
                    {start_idx, YELLOW}, {end_idx, YELLOW}, {left_ptr, RED}, {right_ptr, RED}, {pivot, GREEN}};
                display(arr, color_map);
            }
...
void heapify(std::vector<uint16_t> &arr, int n, int i,
             std::function<void(std::vector<uint16_t>,
                                std::unordered_map<uint16_t, std::string>)>
                 display)
{
